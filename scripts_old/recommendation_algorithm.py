...
def get_price_trend(days):
    ...
    bitcoin_price = requests.get(f'https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days={days}').json()
...
def make_recommendation():
    try:
        ...
        if percentage_change > 5 and long_term_trend > 2 and sentiment > 60:
            return ({'message': f'Bitcoin price has increased by {percentage_change:.2f}% since the last recommendation, long-term trend is {long_term_trend:.2f}% positive, and market sentiment is {sentiment:.2f}% positive. It is recommended to sell.'}, None)
        elif percentage_change < -5 and long_term_trend < -2 and sentiment < 40:
            return ({'message': f'Bitcoin price has decreased by {abs(percentage_change):.2f}% since the last recommendation, long-term trend is {long_term_trend:.2f}% negative, and market sentiment is {sentiment:.2f}% negative. It is recommended to buy.'}, None)
        else:
